...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_APP_SERVER_URL}api/v1/application/apply/${params.id}`);
      if (res.data.success) {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`${import.meta.env.VITE_APP_SERVER_URL}api/v1/job/${params.id}`);
        if (res.data.success) {
